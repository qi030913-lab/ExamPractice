...
public class PaperManager {
    ...
    public void showAddPaperDialog() {
        ...
        questionPanel.setBorder(BorderFactory.createTitledBorder("选择题目"));
        ...
        questionPanel.add(selectedCountLabel, BorderLayout.NORTH);

        JButton selectQuestionButton = new JButton("选择题目");
        selectQuestionButton.setFont(new Font("微软雅黑", Font.PLAIN, 12));
        selectQuestionButton.setBackground(UIUtil.PRIMARY_COLOR);
        selectQuestionButton.setForeground(Color.WHITE);
        selectQuestionButton.setFocusPainted(false);
        selectQuestionButton.setBorder(BorderFactory.createEmptyBorder(8, 20, 8, 20));
        
        java.util.List<Integer> selectedQuestionIds = new java.util.ArrayList<>();
        selectQuestionButton.addActionListener(e -> showQuestionSelectionDialog(dialog, selectedQuestionIds, selectedCountLabel));
        questionPanel.add(selectQuestionButton, BorderLayout.CENTER);
        ...
        JButton confirmButton = new JButton("创建试卷");
    ...
    private void showQuestionSelectionDialog(JDialog parentDialog, java.util.List<Integer> selectedQuestionIds, JLabel selectedCountLabel) {
        ...
        // 表格
        String[] columns = {"选择", "ID", "题目内容", "类型", "难度", "分值", "选项A", "选项B", "选项C", "选项D", "正确答案"};
        ...
        questionTable.setFont(new Font("微软雅黑", Font.PLAIN, 12));
        questionTable.setRowHeight(25);
        questionTable.getTableHeader().setFont(new Font("微软雅黑", Font.BOLD, 12));
        ...
        // 设置选择列的渲染器和编辑器
        questionTable.getColumnModel().getColumn(0).setCellRenderer(new PaperButtonEditor.QuestionCheckBoxRenderer());
        questionTable.getColumnModel().getColumn(0).setCellEditor(new PaperButtonEditor.QuestionCheckBoxEditor());

        // 加载题目数据
        try {
            List<Question> questions = mainFrame.getQuestionService().getAllQuestions();
            for (Question q : questions) {
                Object[] row = {
                    selectedQuestionIds.contains(q.getQuestionId()),
                    q.getQuestionId(),
                    UIUtil.truncate(q.getContent(), 30),
                    q.getQuestionType().getDescription(),
                    q.getDifficulty().getDescription(),
                    q.getScore(),
                    UIUtil.truncate(q.getOptionA(), 20),
                    UIUtil.truncate(q.getOptionB(), 20),
                    UIUtil.truncate(q.getOptionC(), 20),
                    UIUtil.truncate(q.getOptionD(), 20),
                    q.getCorrectAnswer()
                };
        ...
        confirmButton.setFont(new Font("微软雅黑", Font.PLAIN, 12));
        confirmButton.setBackground(UIUtil.PRIMARY_COLOR);
        confirmButton.setForeground(Color.WHITE);
        confirmButton.setFocusPainted(false);
        confirmButton.setBorder(BorderFactory.createEmptyBorder(8, 20, 8, 20));
        confirmButton.addActionListener(e -> {
