...
# 查找jar文件
JAR_FILE=""
if [ -f "target/exam-student.jar" ]; then
    JAR_FILE="target/exam-student.jar"
elif [ -f "exam-student.jar" ]; then
    JAR_FILE="exam-student.jar"
else
    osascript -e 'display dialog "未找到学生端jar文件！\n请先执行 mvn package 构建项目。" buttons {"确定"} default button 1 with title "错误" with icon stop'
    exit 1
fi
